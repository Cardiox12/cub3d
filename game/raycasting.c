...
/*   Updated: 2020/05/10 00:39:48 by tony             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_texture(t_game *data, int x)
{
	...
	data->camera.tex.x = (int)(data->camera.wallX * texture->width);
    ...
    while (y < data->camera.draw_end)
    {
        ...
        color = get_color(&texture->image, data->camera.tex);
        
        if (data->camera.side == 1)
            color = (color >> 1) & 8355711;
        set_color(data, x, y, color);
        y++;
    }
}
