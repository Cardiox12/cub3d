...
/*   Updated: 2020/06/05 16:59:30 by bbellavi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bitmap	create_bitmap(t_vec res)
{
	...
	bitmap = (t_bitmap){
		.bf_type = MAGIC_VALUE,
		.bf_size = res.x * res.y +
		BMP_IMAGE_HEADER_SIZE + BMP_FILES_HEADER_SIZE,
		.bf_reserved1 = 0,
		.bf_reserved2 = 0,
		.bf_offset_bits = 54,
...
int			export_to_bmp(const char *filename, t_game *data)
{
	...
	fd = open(filename, O_CREAT | O_WRONLY | O_TRUNC | O_APPEND, S_IRWXU);
	if (fd < 0)
		return (1);
	...
	while (i.y < data->map.resolution.y)
	{
		...
		write(
			fd,
			&data->image.img_data_addr[i.x],
			sizeof(unsigned int) * data->map.resolution.x);
		i.y++;
	}
	return (0);
}
