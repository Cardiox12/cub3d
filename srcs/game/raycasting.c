...
/*   Updated: 2020/06/05 21:41:42 by bbellavi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	perform_dda(t_game *data)
{
	while (TRUE)
	{
		...
		if (is_wall(
			data->map.map[data->camera.map_pos.y][data->camera.map_pos.x]))
			return ;
	}
}
...
static void	get_perp_dist(t_game *data)
{
	...
	{
		data->camera.perp_wall_dist = (data->camera.map_pos.x
		- data->camera.pos.x + (1 - data->camera.step.x) / 2)
		/ data->camera.ray_dir.x;
	}
	else
	{
		data->camera.perp_wall_dist = (data->camera.map_pos.y
		- data->camera.pos.y + (1 - data->camera.step.y) / 2)
...
static void	init_raycast_variables(t_game *data, int x)
{
	...
	data->camera.ray_dir.x = data->camera.plan_front.x
	+ data->camera.plan_right.x * data->camera.camera_x;
	data->camera.ray_dir.y = data->camera.plan_front.y
	+ data->camera.plan_right.y * data->camera.camera_x;
	data->camera.map_pos = to_vec(data->camera.pos);
	data->camera.ray_dir.x = (data->camera.ray_dir.x == 0)
	? 1 : data->camera.ray_dir.x;
	data->camera.ray_dir.y = (data->camera.ray_dir.y == 0)
	? 1 : data->camera.ray_dir.y;
